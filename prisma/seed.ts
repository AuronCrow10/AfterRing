...
async function main() {
  if (!TWILIO_INBOUND_NUMBER || !TWILIO_INBOUND_NUMBER.startsWith('+')) {
    throw new Error(
      'Please set TWILIO_INBOUND_NUMBER in .env to your real Twilio number in E.164 format.'
    );
  }
  ...
  // ----------------------------------------------------
  // 5. Demo call sessions and recall leads
  // ----------------------------------------------------
  ...
  for (const [index, demo] of demoCalls.entries()) {
    const callSession = await prisma.callSession.upsert({
      ...
      create: {
        ...
        endedAt: new Date(Date.now() - (index + 1) * 60 * 60 * 1000 + (90 + index * 45) * 1000),
        billableSeconds: 90 + index * 45,
        status: 'completed'
      }
    });
